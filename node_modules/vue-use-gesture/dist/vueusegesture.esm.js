import { onMounted, ref, watch, onUnmounted, toRefs, reactive, computed } from 'vue-demi';
...
var Recognizer = /*#__PURE__*/function () {
  ...
  _proto.getMovement = function getMovement(values) {
    ...
    if (_intentional[1] !== false && wasIntentional[1] === false) {
      ...
      __cachedInitial = (_cachedInitial = __cachedInitial) !== null && _cachedInitial !== void 0 ? _cachedInitial : valueFn(initial, this.state);
      __cachedBounds = (_cachedBounds = __cachedBounds) !== null && _cachedBounds !== void 0 ? _cachedBounds : valueFn(bounds, this.state);
...
function useRecognizers(handlers, config, nativeHandlers) {
  ...
  var classes = resolveClasses(handlers); // const controller = React.useMemo(() => new Controller(classes), []);

  var controller = new Controller(classes);
  controller.config = config;
  controller.handlers = handlers;
  controller.nativeRefs = nativeHandlers; // React.useEffect(controller.effect, []);
  // console.log(controller.effect);//

  onMounted(controller.effect); // @ts-ignore
...
function getTwoTouchesEventData(event) {
  ...
  var e = 'nativeEvent' in event ? event['nativeEvent'] : event; //TODO: why never?

  var distance = Math.hypot(dx, dy);
  var angle = (_e$rotation = e.rotation) !== null && _e$rotation !== void 0 ? _e$rotation : -(Math.atan2(dx, dy) * 180) / Math.PI;
...
var raf = isClient ? /*#__PURE__*/window.requestAnimationFrame.bind(window) : function () {};
var cancelRaf = isClient ? /*#__PURE__*/window.cancelAnimationFrame.bind(window) : function () {};
var now = typeof performance !== 'undefined' ? /*#__PURE__*/performance.now.bind(performance) : /*#__PURE__*/Date.now.bind(Date);
var isArray = /*#__PURE__*/Array.isArray.bind(Array);
...
function stepper(secondPerFrame, x, v, destX, spring) {
  ...
  var Fspring = -spring.tension * (x - destX); // Damping, in kg / s

  var Fdamper = -spring.friction * v;
  var a = (Fspring + Fdamper) / (spring.mass || 1);
  var newV = v + a * secondPerFrame;
  var newX = x + newV * secondPerFrame;
  var precision = spring.precision || 0.01;
...
function defineInitialValues(values, velocities) {
  var newValues = isArray(values) ? [] : {};
  var newVelocities = isArray(values) ? [] : {};

  for (var key in values) {
    newValues[key] = values[key]; // @ts-ignore
...
function useSpring(initialValue, // TODO: could change
springConfiguration, options) {
  if (options === void 0) {
    options = {};
  }

  var spring = springConfiguration || noWobble;
  var onRest = options.onRest || noop$1; // hold a ref to real values so we can change them
  // @ts-ignore

  var realValues = isFunction(initialValue) ? ref(initialValue()) : ref(initialValue);
  var wasAnimating = false;
  var prevTime = 0;
  var accumulatedTime = 0;
  watch(realValues, function (_current, _old) {
    if (!wasAnimating) {
      prevTime = now();
      accumulatedTime = 0;
      animate();
  ...
  var current = defineInitialValues(realValues.value, null);
  var currentValues = ref(current[0]);
  var currentVelocities = ref(current[1]); // only done to correctly infer types, they're overridden in mounted

  var idealValues = current[0];
  var idealVelocities = current[1];
  onMounted(function () {
    prevTime = now();
    accumulatedTime = 0;
    var ideal = defineInitialValues(currentValues.value, currentVelocities.value);
    idealValues = ideal[0];
    idealVelocities = ideal[1];
    animate();
  });
  var animationId; // TODO: also cancel when a new value comes
  ...
  function animate() {
    animationId = raf(function () {
      ...
      wasAnimating = true; // get time from last frame

      var currentTime = now();
      var timeDelta = currentTime - prevTime;
      prevTime = currentTime;
      accumulatedTime += timeDelta; // more than 10 frames? probably switched browser tab. Restart
      ...
      if (accumulatedTime === 0) {
        ...
        animationId = null; // TODO: emit
        // this.$emit('motion-restart')

        return animate();
      }

      var currentFrameCompletion = (accumulatedTime - Math.floor(accumulatedTime / msPerFrame) * msPerFrame) / msPerFrame;
      var framesToCatchUp = Math.floor(accumulatedTime / msPerFrame);
      animateValues(framesToCatchUp, currentFrameCompletion, spring, realValues.value, currentValues.value, currentVelocities.value, idealValues, idealVelocities); // out of the update loop

      animationId = null; // the amount we're looped over above

      accumulatedTime -= framesToCatchUp * msPerFrame; // keep going!

      animate();
    });
  }

  var modifiableValues = isArray(realValues.value) ? [] : {};
  ...
  var _loop = function _loop(key) {
    ...
    modifiableValues[key] = computed({
      ...
      // @ts-ignore
      set: function set(v) {
        return realValues.value[key] = v;
      }
    });
  };

  for (var key in realValues.value) {
    _loop(key);
  }

  if (isFunction(initialValue)) {
    // @ts-ignore
    var values = toRefs(reactive(modifiableValues));

    var update = function update(rawValuesObject) {
      var keys = Object.keys(rawValuesObject);
      keys.forEach(function (key) {
        if (!(key in values)) return;
        values[key].value = rawValuesObject[key];
        if ('config' in rawValuesObject) Object.assign(spring, {
          config: rawValuesObject.config
        });
        if ('immediate' in rawValuesObject) Object.assign(spring, {
          immediate: rawValuesObject.immediate
        });
      });
    }; // @ts-ignore
...
function shouldStopAnimation(currentValues, values, currentVelocities) {
  for (var key in values) {
    ...
    if (currentVelocities[key] !== 0) return false; // Something hasn't reached its destination
...
function animateValues(framesToCatchUp, currentFrameCompletion, spring, realValues, currentValues, currentVelocities, idealValues, idealVelocities) {
  for (var key in realValues) {
    ...
    var value = realValues[key]; // iterate as if the animation took place

    for (var i = 0; i < framesToCatchUp; i++) {

      var _stepper = stepper(msPerFrame / 1000, newIdealValue, newIdealVelocity, value, spring);

      newIdealValue = _stepper[0];
      newIdealVelocity = _stepper[1];
    }

    var _stepper2 = stepper(msPerFrame / 1000, newIdealValue, newIdealVelocity, value, spring),
        nextIdealValue = _stepper2[0],
        nextIdealVelocity = _stepper2[1]; // @ts-ignore


    currentValues[key] = newIdealValue + (nextIdealValue - newIdealValue) * currentFrameCompletion; // @ts-ignore

    currentVelocities[key] = newIdealVelocity + (nextIdealVelocity - newIdealVelocity) * currentFrameCompletion; // @ts-ignore

    idealValues[key] = newIdealValue; // @ts-ignore

    idealVelocities[key] = newIdealVelocity;
  }
}

export { addV, gentle, noWobble, presets, rubberbandIfOutOfBounds, stiff, subV, useDrag, useGesture, useHover, useMove, usePinch, useScroll, useSpring, useWheel, wobbly };
//# sourceMappingURL=vueusegesture.esm.js.map
